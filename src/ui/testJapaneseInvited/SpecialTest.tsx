...
export function SpecialTest({ questionCount, onBackToSelect }: SpecialTestProps) {
  ...
  const characterMap = 토로햄전용시험지;
  const testTitle = '토로햄전용';
  ...
  useEffect(() => {
    if (!isFinished && selectedAnswer === '' && questionTimeLeft > 0) {
      const timer = setInterval(() => {
        setQuestionTimeLeft(prev => {
          if (prev <= 1) {
            const currentQuestion = questions[currentQuestionIndex];
            setWrongAnswers(prevWrong => [...prevWrong, currentQuestion]);
  ...
  }, [isFinished, selectedAnswer, questionTimeLeft, currentQuestionIndex, questions]);
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '10px', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>문제 {currentQuestionIndex + 1} / {questions.length}</span>
        <div style={{ display: 'flex', gap: '20px' }}>
          <span style={{ color: questionTimeLeft <= 2 ? '#dc3545' : '#007bff', fontWeight: 'bold' }}>
            문제 시간: {questionTimeLeft}초
          </span>
          <span style={{ color: timeLeft < 300 ? '#dc3545' : '#007bff', fontWeight: 'bold' }}>
